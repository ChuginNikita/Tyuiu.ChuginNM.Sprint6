...
namespace Tyuiu.ChuginNM.Sprint6.Task2.V20
{
    public partial class FormMain : Form
    {
        ...
        private void buttonDone_Click(object sender, EventArgs e)
        {
            try
            {
                int startStep = Convert.ToInt32(textBoxStart_CNM.Text);
                int stopStep = Convert.ToInt32(textBoxEnd_CNM.Text);
                ...
                this.chartResult_CNM.Titles.Add("График функции");

                this.chartResult_CNM.ChartAreas[0].AxisX.Title = "Ось X";
                this.chartResult_CNM.ChartAreas[0].AxisY.Title = "Ось Y";

                for (int i = 0; i <= len - 1; i++)
                {
                    this.dataGridViewResult_CNM.Rows.Add(Convert.ToString(startStep), Convert.ToString(valueArray[i]));

                    this.chartResult_CNM.Series[0].Points.AddXY(startStep, valueArray[i]);
